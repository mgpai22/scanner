# @rosen-bridge/handshake-observation-extractor

## 1.0.0

### Major Changes

- Any service that uses this package requires to initialize the `AddressManager`, provided by `@rosen-bridge/address-manager`, so the Rosen extractor becomes functional

### Patch Changes

- Update dependencies
  - @rosen-bridge/abstract-observation-extractor@1.0.5
  - @rosen-bridge/rosen-extractor@12.0.1
  - @rosen-bridge/tokens@6.0.1

## 0.1.1

### Patch Changes

- Update dependencies
  - @rosen-bridge/abstract-observation-extractor@1.0.4
  - @rosen-bridge/handshake-scanner@0.1.1

## 0.1.0

### Minor Changes

- Add Handshake chain RPC observation extractor
